...
- `GET /api/v1/apps/{app_id}/containers`
- `GET /api/v1/apps/{app_id}/containers/{container_id}`
...
- Dashboard `Project Workspace -> Containers` now also renders an all-project runtime inventory by iterating the existing per-app container endpoints (`GET /api/v1/apps` + `GET /api/v1/apps/{app_id}/containers`), then reuses the same selected-app detail/log endpoints for drill-down; no new API routes or response fields were introduced.
- All-project inventory polling now uses bounded request concurrency and short-circuits while the all-project panel is collapsed; this is a client-side load-control change only and does not alter API schema/semantics.
...
- Containers-tab client now also suspends SSE/reconnect activity while the panel is hidden and resets reconnect backoff after successful live-log events.
